...
func (api *API) watchWorkspaceAgentMetadata(rw http.ResponseWriter, r *http.Request) {
	...
	// We used to use a debounce here, but when the rate of metadata updates was too
	// high the debounce would never fire.
	pubsubRatelimit := rate.NewLimiter(rate.Every(time.Second), 1)
	if flag.Lookup("test.v") != nil {
		pubsubRatelimit = rate.NewLimiter(rate.Every(time.Millisecond*100), 1)
	}
